...
function ConfirmationContent() {
  ...
  useEffect(() => {
    ...
    fetch(`/api/checkout/session?session_id=${encodeURIComponent(sessionId)}`)
      .then((r) => r.json())
      .then((data: { amount?: number; currency?: string; quantity?: number }) => {
        if (data.amount && data.currency) {
          const label = formatAmount(data.amount, data.currency);
          setAmountLabel(
            data.quantity && data.quantity > 1
              ? `${label} (${data.quantity} utilisateurs)`
              : label
          );
        }
      })
      .catch(() => {/* silencieux — on affiche juste sans montant */});
  }, [sessionId]);
