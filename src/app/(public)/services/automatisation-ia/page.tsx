...
import {
  ...
  Lightbulb, TrendingUp, Shield, Briefcase,
} from "lucide-react";
...
const CE_QUE_NOUS_AUTOMATISONS = [
  { icon: MessageSquare, color: "#60a5fa", rgb: "96,165,250",   title: "Réponses clients",           desc: "Chatbots, réponses automatiques aux questions fréquentes, gestion des demandes entrantes — disponible 24h/24." },
  { icon: Users,         color: ACCENT,    rgb: ACCENT_RGB,     title: "Qualification de prospects",  desc: "Tri automatique des leads, scoring, relances programmées — concentrez-vous sur les prospects chauds." },
  { icon: FileText,      color: "#4ade80", rgb: "74,222,128",   title: "Génération de devis",         desc: "Création automatique de devis personnalisés à partir de formulaires ou de données clients." },
  { icon: Settings,      color: "#f9a826", rgb: "249,168,38",   title: "Organisation interne",       desc: "Synchronisation d'outils, notifications automatiques, gestion de tâches et workflow équipe." },
  { icon: Clock,         color: "#f87171", rgb: "248,113,113",  title: "Tâches répétitives",         desc: "Export de données, reporting, envois périodiques, classement automatique — libérez du temps humain." },
  { icon: BrainCircuit,  color: "#f472b6", rgb: "244,114,182",  title: "Assistants IA métier",       desc: "Outils IA spécialisés à votre activité : aide à la rédaction, analyse de données, support interne." },
];

const CAS_USAGE = [
  { icon: Briefcase,  color: "#60a5fa", rgb: "96,165,250", title: "E-commerce",             desc: "Suivi des commandes automatisé, relances paniers abandonnés, recommandations produits personnalisées." },
  { icon: Users,      color: ACCENT,    rgb: ACCENT_RGB,   title: "Agences & freelances",   desc: "Gestion automatique des devis entrants, suivi de projet, rappels clients et reporting hebdomadaire." },
  { icon: BarChart3,  color: "#4ade80", rgb: "74,222,128", title: "Commerces & services",   desc: "Prise de RDV automatisée, confirmations par SMS, gestion des avis clients et des relances." },
  { icon: FileText,   color: "#f9a826", rgb: "249,168,38", title: "Cabinets & professions", desc: "Automatisation des rappels, classement des dossiers, génération de documents récurrents." },
];

const ACCOMPAGNEMENT = [
  { num: "01", icon: MessageSquare, color: "#c9a55a", rgb: "201,165,90", title: "Échange initial",           desc: "On identifie ensemble les tâches qui vous font perdre du temps et les processus à optimiser en priorité." },
  { num: "02", icon: Lightbulb,     color: "#60a5fa", rgb: "96,165,250", title: "Conception de la solution", desc: "On vous propose un plan d'automatisation adapté à vos outils existants et à votre budget." },
  { num: "03", icon: Settings,      color: ACCENT,    rgb: ACCENT_RGB,   title: "Mise en place",             desc: "Développement, paramétrage et intégration de la solution dans votre environnement de travail." },
  { num: "04", icon: CheckCircle2,  color: "#4ade80", rgb: "74,222,128", title: "Test & suivi",              desc: "Vérification du bon fonctionnement, ajustements et accompagnement post-déploiement." },
];

const POURQUOI = [
  { icon: Clock,      color: "#60a5fa", rgb: "96,165,250", title: "Gain de temps immédiat",      desc: "Les tâches automatisées tournent sans vous — vous récupérez des heures chaque semaine." },
  { icon: TrendingUp, color: ACCENT,    rgb: ACCENT_RGB,   title: "Moins d'erreurs humaines",    desc: "Les processus automatisés sont cohérents, fiables et répétables à l'infini." },
  { icon: BarChart3,  color: "#4ade80", rgb: "74,222,128", title: "Meilleure réactivité",        desc: "Réponses instantanées, relances dans les délais, aucune demande client ne tombe dans l'oubli." },
  { icon: Shield,     color: "#f9a826", rgb: "249,168,38", title: "Solutions sur mesure",        desc: "Pas de logiciel générique : on construit ce qui correspond exactement à votre activité." },
];

const CE_QUE_VOUS_OBTENEZ = [
  { label: "Audit des processus",         desc: "Analyse complète de vos flux de travail pour identifier les points automatisables.",           icon: Settings,     color: "129,140,248" },
  { label: "Conception sur mesure",       desc: "Architecture de la solution adaptée à vos outils existants (CRM, formulaires, email…).",       icon: Lightbulb,    color: "96,165,250"  },
  { label: "Développement & déploiement", desc: "Mise en place complète, tests et intégration dans votre environnement de travail.",             icon: BrainCircuit, color: "74,222,128"  },
  { label: "Documentation",               desc: "Guide d'utilisation clair pour que vous puissiez comprendre et piloter la solution.",           icon: FileText,     color: "249,168,38"  },
  { label: "Support & évolutions",        desc: "Suivi post-déploiement, corrections et adaptation de la solution selon vos retours.",           icon: Shield,       color: "244,114,182" },
];

const EXEMPLES_PROJETS = [
  { icon: MessageSquare, color: "#818cf8", rgb: "129,140,248", titre: "Chatbot de qualification",    desc: "Bot conversationnel qui qualifie automatiquement les leads entrants et planifie des RDV dans le calendrier.",       resultat: "+40% de leads traités sans effort humain" },
  { icon: FileText,      color: "#60a5fa", rgb: "96,165,250",  titre: "Génération auto de devis",    desc: "Formulaire intelligent qui génère un devis PDF personnalisé et l'envoie par email en moins de 2 minutes.",          resultat: "Devis envoyé en 2 min au lieu de 30" },
  { icon: BarChart3,     color: "#4ade80", rgb: "74,222,128",  titre: "Reporting hebdomadaire auto", desc: "Collecte et synthèse automatique des données de vente, envoi d'un rapport Slack chaque lundi matin à l'équipe.",    resultat: "3h/semaine économisées sur les reportings" },
];

const FAQ_ITEMS = [
  { q: "Dois-je être technique pour utiliser ces solutions ?", a: "Non. Nous concevons des outils accessibles, avec interface simple. Vous n'avez pas besoin de compétences techniques pour utiliser ce que nous livrons." },
  { q: "Quels outils utilisez-vous pour automatiser ?", a: "Nous travaillons avec des outils comme Make (Integromat), Zapier, n8n, des APIs, et des modèles de langage (LLM) selon les besoins du projet." },
  { q: "Est-ce que ça fonctionne avec mes outils actuels ?", a: "Dans la plupart des cas oui. Lors de l'échange initial, on vérifie la compatibilité avec vos outils existants (CRM, email, agenda, formulaires, etc.)." },
  { q: "Combien coûte une automatisation ?", a: "Le coût dépend de la complexité du projet. Certaines automatisations simples peuvent être mises en place en quelques heures. Contactez-nous pour un devis gratuit." },
  { q: "Puis-je modifier ou faire évoluer la solution après ?", a: "Oui. Toutes les solutions que nous livrons sont documentées et évolutives. Nous pouvons les faire évoluer selon vos besoins." },
];

const ACTIVITE_OPTIONS = ["E-commerce", "Agence / Freelance", "Commerce / Service", "Cabinet / Profession libérale", "Startup / Tech", "Autre"];
...
function FaqItem({ q, a, open, onToggle }: { q: string; a: string; open: boolean; onToggle: () => void }) {
  return (
    <div className="cursor-pointer rounded-2xl border border-white/[0.07] bg-white transition-all duration-200 hover:border-[rgba(129,140,248,0.25)] hover:shadow-sm" onClick={onToggle}>
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      const res = await fetch("/api/contact", { method: "POST", headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ name: nom, email, phone: tel, source: "devis", subject: `Automatisation & IA — ${activite}`, message }) });
    ...
    <motion.form onSubmit={handleSubmit} initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }}
      ...
      <div className="rounded-2xl border bg-white/[0.04] transition-all duration-200"
        ...
        <div className="flex items-start gap-3 px-4 pt-4">
          ...
          <textarea placeholder="Décrivez ce que vous souhaitez automatiser (tâche, outil, contexte, volume…)" value={message}
      ...
      <button type="submit" disabled={!canSubmit || sending}
        ...
        {sending ? <><Loader2 size={17} className="animate-spin" /> Envoi en cours…</> : <><Send size={17} /> Parler de mon automatisation</>}
      </button>
    ...
    <>
      ...
      <main>
          ...
          <div className="relative mx-auto max-w-4xl px-6 text-center">
            ...
            <FadeReveal delay={0.2}>
              <p className="mx-auto mb-10 max-w-2xl text-base leading-relaxed text-white/55 sm:text-lg">
                Nous aidons les entreprises à automatiser certaines tâches, gagner du temps et structurer leurs processus avec des solutions IA adaptées.
              </p>
            </FadeReveal>
            <motion.div variants={staggerContainerFast} initial="hidden" animate="show" className="mb-10 flex flex-wrap justify-center gap-4">
              {[{ label: "Sur mesure", sub: "adapté à vos outils" }, { label: "Rapide à déployer", sub: "quelques jours" }, { label: "ROI mesurable", sub: "temps récupéré" }].map(({ label, sub }) => (
            ...
            <motion.div {...fadeIn} transition={{ delay: 0.35 }}>
              <Link href="#devis" className="btn-primary px-8 py-4 text-base">Parler de mon automatisation <ArrowRight size={16} /></Link>
            </motion.div>
          </div>
        </section>
        ...
        <section className="bg-[#0d0d1a] py-14 sm:py-24">
          <div className="mx-auto max-w-6xl px-6">
            <motion.div variants={staggerContainer} initial="hidden" whileInView="show" viewport={viewport} className="mb-14 text-center">
              <motion.p variants={fadeIn} className="mb-3 text-xs font-semibold uppercase tracking-widest" style={{ color: ACCENT }}>Nos domaines</motion.p>
              ...
              <motion.p variants={fadeIn} className="mt-4 text-sm text-white/45 max-w-xl mx-auto">Des solutions concrètes pour chaque type de tâche répétitive dans votre activité.</motion.p>
            </motion.div>
            <motion.div variants={staggerContainerFast} initial="hidden" whileInView="show" viewport={viewport} className="grid gap-5 sm:grid-cols-2 lg:grid-cols-3">
              {CE_QUE_NOUS_AUTOMATISONS.map(({ icon: Icon, color, rgb, title, desc }) => (
        ...
        <section className="bg-[#09090b] py-14 sm:py-24">
          <div className="mx-auto max-w-6xl px-6">
            <motion.div variants={staggerContainer} initial="hidden" whileInView="show" viewport={viewport} className="mb-14 text-center">
              <motion.p variants={fadeIn} className="mb-3 text-xs font-semibold uppercase tracking-widest" style={{ color: ACCENT }}>Exemples concrets</motion.p>
              <motion.h2 variants={fadeIn} className="text-3xl font-extrabold text-white sm:text-4xl">Exemples de cas d'usage</motion.h2>
            </motion.div>
            <motion.div variants={staggerContainerFast} initial="hidden" whileInView="show" viewport={viewport} className="grid gap-5 sm:grid-cols-2 lg:grid-cols-4">
              {CAS_USAGE.map(({ icon: Icon, color, rgb, title, desc }) => (
                <motion.div key={title} variants={cardReveal} className="group rounded-3xl border border-white/[0.07] bg-white/[0.03] p-6 transition-all duration-300 hover:border-white/[0.12] hover:bg-white/[0.05]">
                  <div className="mb-4 flex h-11 w-11 items-center justify-center rounded-2xl" style={{ background: `rgba(${rgb},0.1)` }}><Icon size={20} style={{ color }} /></div>
                  <h3 className="mb-2 text-sm font-bold text-white">{title}</h3>
                  <p className="text-xs leading-relaxed text-white/45">{desc}</p>
        ...
        <section className="bg-[#0d0d1a] py-14 sm:py-24">
          <div className="mx-auto max-w-5xl px-6">
            <motion.div variants={staggerContainer} initial="hidden" whileInView="show" viewport={viewport} className="mb-14 text-center">
              ...
              <motion.h2 variants={fadeIn} className="text-3xl font-extrabold text-white sm:text-4xl">Comment se passe l'accompagnement</motion.h2>
            </motion.div>
            <motion.div variants={staggerContainerFast} initial="hidden" whileInView="show" viewport={viewport} className="space-y-6">
              {ACCOMPAGNEMENT.map(({ num, icon: Icon, color, rgb, title, desc }) => (
        ...
        <section className="bg-[#09090b] py-14 sm:py-24">
          <div className="mx-auto max-w-6xl px-6">
            <motion.div variants={staggerContainer} initial="hidden" whileInView="show" viewport={viewport} className="mb-14 text-center">
              ...
              <motion.h2 variants={fadeIn} className="text-3xl font-extrabold text-white sm:text-4xl">Pourquoi automatiser avec DJAMA</motion.h2>
            </motion.div>
            <motion.div variants={staggerContainerFast} initial="hidden" whileInView="show" viewport={viewport} className="grid gap-5 sm:grid-cols-2 lg:grid-cols-4">
        ...
        {/* FORMULAIRE */}
        <section id="devis" className="bg-[#0d0d1a] py-14 sm:py-24">
          <div className="mx-auto max-w-2xl px-6">
            <motion.div variants={staggerContainer} initial="hidden" whileInView="show" viewport={viewport} className="mb-10 text-center">
              ...
              <motion.p variants={fadeIn} className="mt-4 text-sm text-white/45">Décrivez ce que vous voulez automatiser — on analyse et on revient vers vous sous 24h.</motion.p>
            </motion.div>
            <DevisForm />
          </div>
        </section>
        ...
        {/* CTA */}
        <section className="relative overflow-hidden bg-[#0d0d1a] pb-14 pt-14 sm:pb-24 sm:pt-20">
          <div className="pointer-events-none absolute inset-0">
          ...
          <div className="relative mx-auto max-w-2xl px-6 text-center">
            <motion.div initial={{ opacity: 0, y: 24 }} whileInView={{ opacity: 1, y: 0 }} viewport={viewport} transition={{ duration: 0.6, ease }}>
              ...
              <h2 className="mb-4 text-3xl font-extrabold text-white sm:text-4xl">Prêt à gagner du temps ?</h2>
              <p className="mb-8 text-sm leading-relaxed text-white/50 max-w-md mx-auto">Dites-nous ce qui vous fait perdre du temps — on construit la solution qui vous libère.</p>
