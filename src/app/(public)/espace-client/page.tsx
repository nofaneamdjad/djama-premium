            ...
            {TOOLS.map((tool) => {
              ...
              return (
                <motion.div
                  ...
                  className="flex flex-col items-center gap-2.5 rounded-2xl border border-gray-100 bg-gray-50 p-4 text-center transition-all hover:-translate-y-0.5 hover:border-gray-200 hover:shadow-sm"
                >
                  <div
                    className="flex h-9 w-9 items-center justify-center rounded-xl border"
                    style={{ background: `rgba(${tool.rgb},0.12)`, borderColor: `rgba(${tool.rgb},0.25)` }}
                  >
                    <Icon size={16} style={{ color: tool.color }} />
                  </div>
                  <p className="text-[0.72rem] font-bold leading-tight text-gray-800">{tool.title}</p>
                </motion.div>
              );
            })}
          </motion.div>
