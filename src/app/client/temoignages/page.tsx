      ...
      {[1,2,3,4,5].map(i => (
        ...
        >
          <Star
            ...
            style={{ color: (hover || value) >= i ? "#c9a55a" : "rgba(255,255,255,0.2)" }}
          />
        </button>
      ))}
    </div>
...
export default function TemoignagesPage() {
  ...
  async function handleSave() {
    ...
    if (selected) {
      ...
      const { error } = await supabase.from("testimonials").insert({ ...draft, user_id: user.id });
      if (error) { showToast("error", error.message); setSaving(false); return; }
    }
  ...
  return (
    <div className="flex flex-col bg-[#080a0f] min-h-screen">
      <div className="pointer-events-none fixed inset-0 z-0">
        <div className="absolute right-[10%] top-[5%] h-[400px] w-[400px] rounded-full bg-[rgba(251,113,133,0.04)] blur-[130px]" />
      </div>
      ...
      <div className="relative z-10 border-b border-white/6 bg-[rgba(15,17,23,0.88)] px-5 py-3.5 backdrop-blur-xl sm:px-8">
        <div className="mx-auto flex max-w-7xl items-center justify-between">
          <div className="flex items-center gap-3">
            <div className="flex h-9 w-9 items-center justify-center rounded-xl border border-[rgba(251,113,133,0.2)] bg-[rgba(251,113,133,0.08)]">
          ...
          <button onClick={newItem}
            className="flex items-center gap-2 rounded-xl bg-gradient-to-r from-[#fb7185] to-[#e11d48] px-4 py-2 text-xs font-extrabold text-white shadow-[0_4px_16px_rgba(251,113,133,0.3)] transition hover:opacity-90">
          ...
          <div className="border-b border-white/6 p-4">
            <div className="relative">
              ...
              <input value={query} onChange={e => setQuery(e.target.value)} placeholder="Rechercher…"
                className="w-full rounded-xl border border-white/8 bg-white/5 py-2.5 pl-9 pr-3 text-sm text-white placeholder:text-white/25 outline-none" />
            </div>
          </div>
          <div className="flex-1 overflow-y-auto">
            {loadingAll ? (
              ...
              <div className="flex flex-col items-center gap-3 py-12 text-center">
                ...
                <p className="text-sm text-white/25">Aucun témoignage</p>
              </div>
            ) : (
              filtered.map(t => (
                <button key={t.id} onClick={() => openItem(t)}
                  className={`w-full border-b border-white/5 px-4 py-3.5 text-left transition hover:bg-white/4 ${selected?.id === t.id ? "bg-white/6" : ""}`}>
                  <div className="flex items-center justify-between mb-1">
                    <div className="flex gap-0.5">
                      {[1,2,3,4,5].map(i => (
                        <Star key={i} size={9} fill={(t.note ?? 5) >= i ? "#c9a55a" : "none"} style={{ color: (t.note ?? 5) >= i ? "#c9a55a" : "rgba(255,255,255,0.15)" }} />
                      ))}
                    </div>
                  ...
                  <p className="text-sm font-bold text-white/90">{t.nom}</p>
                  <p className="text-[0.65rem] text-white/35">{t.role}{t.entreprise ? ` — ${t.entreprise}` : ""}</p>
        ...
        <main className={`flex flex-1 flex-col overflow-hidden ${mobileView === "list" ? "hidden sm:flex" : "flex"}`}>
          {!draft ? (
            <div className="flex h-full flex-col items-center justify-center gap-4 rounded-[1.5rem] border border-white/8 bg-[rgba(15,17,23,0.4)] p-8 text-center">
              ...
              <p className="text-base font-bold text-white">Sélectionnez ou ajoutez un témoignage</p>
              <button onClick={newItem}
              ...
              <div className="flex flex-wrap items-center gap-2 border-b border-white/6 px-5 py-3">
                ...
                <button onClick={() => upd("published", !draft.published)}
                  className={`flex items-center gap-1.5 rounded-xl border px-3 py-1.5 text-xs font-bold transition ${
                    draft.published ? "border-emerald-500/30 bg-emerald-500/10 text-emerald-400" : "border-white/10 bg-white/5 text-white/40 hover:border-white/20"
                  }`}>
                  {draft.published ? <Eye size={11}/> : <EyeOff size={11}/>}
                  {draft.published ? "Publié" : "Brouillon"}
                </button>
                {dirty && <span className="hidden items-center gap-1.5 rounded-full border border-[rgba(251,113,133,0.2)] bg-[rgba(251,113,133,0.08)] px-2.5 py-1 text-[0.6rem] font-semibold text-[#fb7185] sm:inline-flex">
                ...
                <div className="ml-auto flex items-center gap-2">
                  ...
                  <button onClick={handleSave} disabled={saving || !dirty}
                    className="flex items-center gap-2 rounded-xl bg-gradient-to-r from-[#fb7185] to-[#e11d48] px-4 py-2 text-xs font-extrabold text-white shadow-[0_2px_12px_rgba(251,113,133,0.3)] transition hover:opacity-90 disabled:opacity-40">
                ...
                <div className="mx-auto max-w-2xl space-y-5">
                  ...
                  <div className="grid gap-4 sm:grid-cols-2">
                    <div>
                      ...
                      <input value={draft.nom} onChange={e => upd("nom", e.target.value)} placeholder="Prénom Nom"
                        className="w-full rounded-xl border border-white/10 bg-white/5 px-3.5 py-2.5 text-sm text-white placeholder:text-white/20 outline-none transition hover:border-white/20 focus:border-[rgba(251,113,133,0.4)]"/>
                    </div>
                    <div>
                      ...
                      <input value={draft.role} onChange={e => upd("role", e.target.value)} placeholder="Directeur, Freelance…"
                        className="w-full rounded-xl border border-white/10 bg-white/5 px-3.5 py-2.5 text-sm text-white placeholder:text-white/20 outline-none transition hover:border-white/20"/>
                    </div>
                  </div>
                  ...
                  <div>
                    ...
                    <textarea value={draft.texte} onChange={e => upd("texte", e.target.value)} rows={6}
                      ...
                      className="w-full resize-y rounded-xl border border-white/10 bg-white/5 px-3.5 py-3 text-sm text-white placeholder:text-white/20 outline-none transition hover:border-white/20 focus:border-[rgba(251,113,133,0.4)]"/>
                  </div>

                  {/* Preview */}
                  {draft.texte && (
                    <div className="rounded-[1.25rem] border border-[rgba(251,113,133,0.15)] bg-[rgba(251,113,133,0.04)] p-5">
                      <p className="mb-1 text-[0.6rem] font-bold uppercase tracking-widest text-[#fb7185]/50">Aperçu</p>
                      <div className="flex gap-1 mb-3">
                        {[1,2,3,4,5].map(i => <Star key={i} size={12} fill={(draft.note ?? 5) >= i ? "#c9a55a" : "none"} style={{ color: (draft.note ?? 5) >= i ? "#c9a55a" : "rgba(255,255,255,0.15)" }} />)}
                      </div>
                      <p className="text-sm italic leading-relaxed text-white/70">&ldquo;{draft.texte}&rdquo;</p>
                      <div className="mt-3 flex items-center gap-2">
                        {draft.avatar_url ? (
                          // eslint-disable-next-line @next/next/no-img-element
                          <img src={draft.avatar_url} alt="" className="h-8 w-8 rounded-full object-cover"/>
                        ) : (
                          <div className="flex h-8 w-8 items-center justify-center rounded-full border border-white/10 bg-white/8 text-[0.7rem] font-black text-white/50">
                            {draft.nom?.[0]?.toUpperCase() || "?"}
                          </div>
                        )}
                        <div>
                          <p className="text-xs font-bold text-white/85">{draft.nom || "Nom"}</p>
