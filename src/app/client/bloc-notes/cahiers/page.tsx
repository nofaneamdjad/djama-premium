 ...
 * Stylo/souris par défaut · bouton Mode doigt · pinch-to-zoom + molette
 */
...
export default function CahiersPage() {
  ...
  }, [activeBook]); // re-attach when editor mounts
  ...
  const addPage = useCallback(async () => {
    ...
    const { data:pg } = await supabase.from("notebook_pages")
      ...
      .select().single();
    if (pg) {
      setPages(prev=>[...prev,{...pg as NbPage,strokes:[]}]);
      setPageIdx(pages.length); setStrokes([]); setHistory([]); setDirty(false);
    }
  }, [activeBook, pages]);
  ...
  const switchPage = useCallback((idx:number) => {
    setPageIdx(idx); setStrokes(pages[idx]?.strokes??[]); setHistory([]); setDirty(false); setZoom(1);
  }, [pages]);
        ...
        {loading ? (
          ...
          <div className="flex h-full flex-col items-center justify-center gap-8 text-center px-6">
            <div className="relative h-40 w-28" style={{perspective:"400px"}}>
              ...
              <div className="absolute left-[18px] inset-y-0 right-0 rounded-r-2xl overflow-hidden"
                style={{background:COVERS[0].g,boxShadow:"6px 10px 32px rgba(0,0,0,0.6)"}}>
              ...
              return (
                <motion.div key={nb.id} className="group flex flex-col items-center gap-3">
                  <motion.button
                    ...
                    <button onClick={e=>{e.stopPropagation();setDelBook(nb)}}
            ...
            <motion.div className="flex flex-col items-center gap-3">
              <motion.button onClick={()=>setCreateOpen(true)}
                ...
                <div className="flex flex-col items-center gap-2">
                  <div className="flex h-10 w-10 items-center justify-center rounded-full border border-white/12">
          ...
          <>
            ...
            <motion.div key="modal"
              ...
              <div className="mb-5 flex items-center justify-between">
                <h2 className="text-sm font-black text-white">Nouveau cahier</h2>
                <button onClick={()=>setCreateOpen(false)}
                  className="flex h-7 w-7 items-center justify-center rounded-full bg-white/[0.05] text-white/35 hover:text-white/65 transition">
                  <X size={13}/>
                </button>
              </div>
              ...
              <div className="flex gap-3">
                <button onClick={()=>setCreateOpen(false)}
                  className="flex-1 rounded-2xl border border-white/[0.07] py-3 text-sm font-semibold text-white/38">Annuler</button>
            ...
            <motion.div initial={{scale:0.88,y:24}} animate={{scale:1,y:0}} exit={{scale:0.94,opacity:0}}
              ...
              <div className="mt-5 flex gap-3">
                ...
                <button onClick={()=>void deleteBook()}
                  className="flex-1 rounded-2xl bg-red-500/75 py-2.5 text-sm font-bold text-white hover:bg-red-500 transition">Supprimer</button>
              </div>
            </motion.div>
          </motion.div>
        )}
    ...
    <div className="flex h-[calc(100vh-56px)] flex-col overflow-hidden" style={{background:PAGE_BG}}>
      ...
      <div className="flex items-center gap-2 h-11 px-3 flex-shrink-0 border-b border-black/[0.07] bg-white/80 backdrop-blur-sm">
        ...
        <div className="flex-shrink-0 h-2 w-2 rounded-full transition-colors"
          style={{background:saving?AMBER:dirty?"#f97316":"#22c55e",opacity:saving||dirty?1:0.4}}/>
      ...
      <div className="flex flex-1 overflow-hidden">
          ...
          {thumbsOpen && (
            <motion.div key="thumbs-m"
              ...
              <button onClick={()=>{void addPage();setThumbsOpen(false)}}
                className="w-full flex-shrink-0 flex items-center justify-center rounded-lg border border-dashed border-black/12 hover:border-amber-400/40 transition"
                style={{aspectRatio:"3/4"}}>
        ...
        <div className="hidden lg:flex w-[52px] flex-col border-r border-black/[0.06] bg-white/60 overflow-y-auto p-2 gap-2 flex-shrink-0" style={{scrollbarWidth:"none"}}>
          ...
          <button onClick={()=>void addPage()}
            className="w-full flex-shrink-0 flex items-center justify-center rounded-lg border border-dashed border-black/10 hover:border-amber-400/35 transition"
            style={{aspectRatio:"3/4"}}>
            <Plus size={11} className="text-slate-300"/>
          </button>
        </div>
        ...
        >
          {/* Zoom layer — CSS transform, getBoundingClientRect corrige auto les coords */}
          <div style={{
          ...
          {/* Ombre haute */}
          <div className="pointer-events-none absolute top-0 left-0 right-0 h-5 z-10"
            style={{background:"linear-gradient(to bottom,rgba(0,0,0,0.04),transparent)"}}/>

          {/* Indicateur mode doigt */}
          {fingerMode && (
            <div className="pointer-events-none absolute top-3 right-3 z-10 flex items-center gap-1.5 rounded-full border border-blue-300/40 bg-blue-50/90 px-2.5 py-1 backdrop-blur-sm">
              <Hand size={11} className="text-blue-500"/>
              <span className="text-[0.6rem] font-bold text-blue-600">Mode doigt</span>
            </div>
          )}
          ...
          {zoom !== 1 && (
            <button onClick={()=>setZoom(1)}
              className="pointer-events-auto absolute bottom-3 right-3 z-10 flex items-center gap-1 rounded-full border border-black/[0.1] bg-white/90 px-2.5 py-1 text-[0.6rem] font-black text-slate-600 shadow-sm backdrop-blur-sm transition hover:bg-white">
              {Math.round(zoom*100)}%
            </button>
          )}
      ...
      <div
        ...
        <div className="h-5 w-px flex-shrink-0 bg-black/[0.08] mx-0.5"/>

        {/* Tools */}
        {([
          { t:"pen"         as NbTool, Icon:Pencil },
          { t:"highlighter" as NbTool, Icon:Highlighter },
          { t:"eraser"      as NbTool, Icon:Eraser },
        ]).map(({t,Icon})=>(
          <button key={t} onClick={()=>setTool(t)}
            className={`flex h-9 w-9 flex-shrink-0 items-center justify-center rounded-xl transition ${tool===t
        ...
        {/* Mode doigt */}
        <button onClick={()=>setFingerMode(f=>!f)}
          className={`flex items-center gap-1.5 h-9 px-2.5 flex-shrink-0 rounded-xl text-xs font-bold transition ${fingerMode
            ?"bg-blue-50 text-blue-600":"text-slate-400 hover:bg-black/[0.04] hover:text-slate-600"}`}
          title={fingerMode ? "Désactiver mode doigt" : "Activer mode doigt"}>
          <Hand size={14}/>
          <span className="hidden sm:inline">{fingerMode ? "Doigt On" : "Doigt"}</span>
        </button>
        ...
        {PEN_COLORS.map(c=>(
          <button key={c} onClick={()=>setColor(c)}
            ...
            style={{
              ...
              outlineOffset:"2px",
              boxShadow: c==="#ffffff" ? "inset 0 0 0 1.5px rgba(0,0,0,0.18)" : undefined,
            }}/>
        ))}
        ...
        {WIDTH_DOT.map((r,i)=>(
          <button key={i} onClick={()=>setWIdx(i)}
            ...
            <div className="rounded-full"
              style={{width:r,height:r,background:wIdx===i?"#d97706":"#94a3b8"}}/>
          </button>
        ))}
        ...
        <button onClick={()=>setZoom(z=>Math.max(0.5,+(z-0.25).toFixed(2)))} disabled={zoom<=0.5}
          className="flex h-9 w-9 flex-shrink-0 items-center justify-center rounded-xl text-slate-400 transition hover:bg-black/[0.04] hover:text-slate-700 disabled:opacity-20">
          <ZoomOut size={15}/>
        </button>
        <button onClick={()=>setZoom(1)}
          className="flex-shrink-0 min-w-[38px] text-center text-[0.58rem] font-black text-slate-400 tabular-nums transition hover:text-slate-700">
          {Math.round(zoom*100)}%
        </button>
