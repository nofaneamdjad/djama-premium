...
export default function ClientFacturesPage() {
  ...
  async function saveInvoice() {
    ...
    if (userError || !user) {
      setMessage("Vous devez être connecté pour enregistrer une facture.");
    ...
    const { error } = await supabase.from("factures").insert({
      ...
      entreprise_adresse: companyAddress,
      entreprise_telephone: companyPhone,
      entreprise_siret: companySiret,
      entreprise_tva: companyVatNumber,
      type_document: documentType,
      date_emission: issueDate || null,
      date_echeance: dueDate || null,
      notes,
      conditions_paiement: paymentTerms,
    ...
    if (error) {
      ...
      setMessage("Facture enregistrée avec succès dans Supabase.");
    }
  ...
  return (
    <main className="bg-white">
      <section className="mx-auto max-w-7xl px-6 py-16">
        ...
        {message ? (
          <p className="mt-4 rounded-xl border border-luxe bg-white px-4 py-3 text-sm text-zinc-700">
            {message}
          </p>
        ) : null}
      </section>
          ...
          <div className="rounded-3xl border border-luxe bg-white p-6 shadow-luxe-soft">
            <div className="grid gap-6">
              ...
              <div className="flex flex-wrap gap-4">
                ...
                >
                  {loadingSave ? "Enregistrement..." : "Enregistrer dans Supabase"}
                </button>

                <button
                  type="button"
                  onClick={downloadPDF}
                  className="rounded-2xl border border-luxe bg-white px-6 py-4 text-lg font-extrabold"
                >
                  Télécharger en PDF
                </button>
              </div>
